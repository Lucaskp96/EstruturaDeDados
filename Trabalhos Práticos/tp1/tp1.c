...
bool inserir(Dicionario * dicionario, char * pais, char * frase) {
    if(pesquisar(dicionario,pais) == false){
        dicionario->letras[pais[0] - OFFSET].pais = pais;
        dicionario->letras[pais[0] - OFFSET].frase = frase;
        return true;
    }else{
        return false;
    }
...
char * saudar(Dicionario * dicionario, char * pais) {
    if(pesquisar(dicionario,pais) == true){
